...
- **Duration:** 1:15 - 2:00
- **Overview:** Answer technical and conceptual questions and patch a couple of holes in this toy PoW blockchain.
...
##### [P2]
Implement using the longest-chain alg, or change logic outside of this function to flex your favorite chain selection alg. Remember, we store a block tree as an adjacency matrix in a `HashSet`. 
...
##### [R1]
We use `async_channel` to create `mpmc` Senders and Receivers. Why would sharing a reference to a `mpsc::Receiver` potentially be dangerous? 
